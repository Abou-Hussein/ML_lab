...
# Target model updated every 1000 iteration
def model_target():
...
if(Training):
	for step in range(steps):
		...
		if random.random() <= epsilon or step < explore:
			...
			#print("hehehe", state_with_history.shape)
		...
		if step > observe:
			...
			if(step%100) == 0:
				...
				flagt= False
			if (step % 10000) == 0:
				# Copy new weightes to the target model
	...
	reached = 0
	failed = 0
	json_file = open('fmodel800k.json','r')
	modell = json_file.read()
	json_file.close()
	ml = model_from_json(modell)
	ml.load_weights("fmodel800k.h5")
	print("Model loaded")
	state = sim.newGame(opt.tgt_y, opt.tgt_x)
	steps_test = 1000
	state_with_history = np.zeros((opt.hist_len, opt.state_siz))
