...
if(Training):
	for step in range(steps):
		...
		if step == 600000:
			...
			with open("ddqnr600k.json", "w") as json_file:
				json_file.write(model_json)
			# serialize weights to HDF5
			model_q.save_weights("ddqnr600k.h5")
			print("Saved model-600k to disk")

		if step == 800000:
			model_json = model_q.to_json()
			with open("ddqnr800k.json", "w") as json_file:
				json_file.write(model_json)
			# serialize weights to HDF5
			model_q.save_weights("ddqnr800k.h5")
			print("tarSaved model-800k to disk")
			with open('ddqnloss', 'wb') as ls:
				pickle.dump(ll, ls)
			with open('ddqnrewarddd', 'wb') as rd:
				pickle.dump(re, rd)

		if step == 1000000:
			model_json = model_q.to_json()
			with open("ddqnr1000k.json", "w") as json_file:
				json_file.write(model_json)
			# serialize weights to HDF5
			model_q.save_weights("ddqnr1000k.h5")
