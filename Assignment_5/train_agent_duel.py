...
if(Training):
	for step in range(steps):
		...
		if step == 600000:
			...
			with open("duel600k.json", "w") as json_file:
				json_file.write(model_json)
			# serialize weights to HDF5
			model_q.save_weights("duel600k.h5")
			print("Saved model-600k to disk")

		if step == 800000:
			model_json = model_q.to_json()
			with open("duel800k.json", "w") as json_file:
				json_file.write(model_json)
			# serialize weights to HDF5
			model_q.save_weights("duel800k.h5")
			print("tarSaved model-800k to disk")
			with open('duelloss', 'wb') as ls:
				pickle.dump(ll, ls)
			with open('duelreward', 'wb') as rd:
				pickle.dump(re, rd)

		if step == 1000000:
			model_json = model_q.to_json()
			with open("duel1000k.json", "w") as json_file:
				json_file.write(model_json)
			# serialize weights to HDF5
			model_q.save_weights("duel1000k.h5")
