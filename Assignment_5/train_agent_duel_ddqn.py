...
def model_q():
	...
	x = Convolution2D(32, 8, 8, subsample=(4, 4), activation='relu', border_mode='same',
		input_shape=(ROWS, COLS, CHANNELS_NUM), init='uniform',
		trainable=True)(input)
	x = Convolution2D(64, 4, 4, subsample=(2, 2), activation='relu', border_mode='same', init='uniform', trainable=True)(x)
	x = Convolution2D(64, 3, 3, subsample=(2, 2), activation='relu', border_mode='same', init='uniform', trainable=True)(x)
	x = Flatten()(x)
	# state value tower - V
	state_value = Dense(32, activation='relu', init='uniform')(x)
	...
	action_advantage = Dense(32, activation='relu', init='uniform')(x)
	action_advantage = Dense(opt.act_num, init='uniform')(action_advantage)
...
def model_target():
	...
	# Momken yetla3 error hena 3ashan el far2 been el input shape wel tanya
	input = Input(shape=(ROWS, COLS, CHANNELS_NUM))
	x = Convolution2D(32, 8, 8, subsample=(4, 4), activation='relu', border_mode='same',
		input_shape=(ROWS, COLS, CHANNELS_NUM), init='uniform',
		trainable=True)(input)
...
reward_sum = 0
loss_sum = 0
Training = True # Variable used for testing or training
...
if(Training):
	for step in range(steps):
		...
		if step == 600000:
			...
			with open("dueldd600k.json", "w") as json_file:
				json_file.write(model_json)
			# serialize weights to HDF5
			model_q.save_weights("dueldd600k.h5")
			print("Saved model-600k to disk")

		if step == 800000:
			model_json = model_q.to_json()
			with open("dueldd800k.json", "w") as json_file:
				json_file.write(model_json)
			# serialize weights to HDF5
			model_q.save_weights("dueldd800k.h5")
			print("tarSaved model-800k to disk")
			with open('duelddloss', 'wb') as ls:
				pickle.dump(ll, ls)
			with open('duelddreward', 'wb') as rd:
				pickle.dump(re, rd)

		if step == 1000000:
			model_json = model_q.to_json()
			with open("dueldd1000k.json", "w") as json_file:
				json_file.write(model_json)
			# serialize weights to HDF5
			model_q.save_weights("dueldd1000k.h5")
